...
void ObjManager::SnakeEatFood(unsigned long long id)
{
    ...
    double new_x = tail.m_x - (unit_dx * SEGMENT_SIZE); // ²¿¸®->¾Õ²¿¸® º¤ÅÍ¸¦ »­
    double new_y = tail.m_y - (unit_dy * SEGMENT_SIZE); // ²¿¸®->¾Õ²¿¸® º¤ÅÍ¸¦ »­
...
{
    auto& snake_head = m_snakes[0].body.front(); // ÂüÁ¶¸¦ »ç¿ëÇÏ°í front() »ç¿ë
